...
for day, guard_start in guardlog.items():
    ...
    for entry in daylog[day]:
        ...
        # wake pair
        sleep_record[day]['record'][minute:] = [1^x for x in  sleep_record[day]['record'][minute:]]
...
for day, day_sleep_record in sleep_record.items():
    if day_sleep_record['guard_id'] == gid_max:
        minute_tracker = [a+b for a,b in zip(minute_tracker,day_sleep_record['record'])]
...
for day, day_sleep_record in sleep_record.items():
    ...
    if gid not in minutewise_guard_stats:
        ...
        minutewise_guard_stats[gid] = [a+b for a,b in zip(minutewise_guard_stats[gid],day_sleep_record['record'])]
